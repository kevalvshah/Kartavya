...
| `brand.js` | `KWordmark` React component — the KARTAVYA logo using Harabara Mais font. Only place this font is used. | `AppShell.jsx` (mobile topbar), auth pages |
...
- `api.js` must be the only file that contains the backend URL.
  Never hardcode `https://kartavya-...railway.app` anywhere else.
