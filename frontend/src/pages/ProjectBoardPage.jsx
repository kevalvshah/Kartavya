 ...
 * Kanban | Table | Calendar views + saved views + field manager + column management.
 */
...
function NewTaskModal({ columns, teamId, onSaved, onClose }) {
  const [title,setPriority2]=useState('');
  ...
  const submit=async(e)=>{
    ...
    setSaving(true);
    try{const res=await api.post('/api/tasks',{title:title.trim(),team_id:teamId,column_id:colId,priority});onSaved(res.data);onClose();}
  ...
  return(
    <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <form style={S.box} onSubmit={submit}>
        ...
        <input style={S.input} autoFocus value={title} onChange={e=>setPriority2(e.target.value)} placeholder="Task title…" required />
...
export default function ProjectBoardPage() {
  ...
  const teamMembers=(members||[]).map(m=>({user_id:m.user_id||m.member_id,display_name:m.display_name||m.full_name||m.email}));
  ...
  const S={
    ...
    btn:(v)=>({border:v?'none':'1px solid var(--border-default)',borderRadius:'var(--radius-sm)',padding:'5px 12px',background:v?'var(--accent-default)':'var(--bg-default)',cursor:'pointer',fontFamily:'inherit',fontSize:'var(--text-sm)',color:v?'#fff':'var(--text-muted)',fontWeight:v?700:400}),
  };
  ...
  return(
    <div style={S.page}>
      ...
      {view==='kanban'&&<KanbanView columns={columns} tasks={tasks} fieldDefs={fieldDefs} fieldValueMap={fieldValueMap} teamMembers={teamMembers} onTasksChange={handleTasksChange} onColumnChange={(a,c)=>a==='new_task'&&setShowNew(true)} />}
      ...
      {showNew&&<NewTaskModal columns={columns} teamId={teamId} onSaved={t=>setTasks(p=>[t,...p])} onClose={()=>setShowNew(false)} />}
    </div>
  );
}
