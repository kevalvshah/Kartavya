...
def main():
    ...
    tabla = render(
        partials=[(180, 1.0, 14.0, 60.0), (110, 0.6, 10.0, 20.0), (340, 0.25, 30.0, 0.0)],
        duration=0.4, fade_in=0.002, fade_out=0.1,
    )
    ...
    for i in range(n):
        ...
        f = 523.25 * vibrato  # C5
        env = min(1.0, t / 0.08) * math.exp(-1.2 * t)
    ...
    sitar = render(
        partials=[
            (329.63, 1.0, 4.0, 8.0),   # E4 fundamental, slow decay
            (659.26, 0.5, 7.0, 4.0),   # 2nd harmonic, faster decay
            (989.0,  0.35, 10.0, 2.0), # 3rd harmonic
            (1318.5, 0.2, 14.0, 0.0),  # 4th harmonic (buzz/jhankar)
        ],
        duration=0.8, fade_in=0.003, fade_out=0.25,
    )
    ...
    for i in range(n):
        ...
        env = min(1.0, t / 0.05) * math.exp(-0.9 * t)
        v = (
    ...
    ghanta = render(
        partials=[
            (440.0, 1.0, 2.2, 0.0),
            (440.0 * 2.4, 0.5, 3.0, 0.0),
            (440.0 * 3.1, 0.35, 3.6, 0.0),
            (440.0 * 4.6, 0.2, 4.5, 0.0),
        ],
        duration=1.1, fade_in=0.002, fade_out=0.4,
    )
